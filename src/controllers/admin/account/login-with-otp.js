    ...
    try {
        ...
        if (result) {
            ...
            return bcryptjs.compare(password, result.password).then(async () => {
                ...
                let findExistingOTP = await OtpRepo.findOne({ user_id: result._id, isActive: true, via: payload.via, send_to: email })
                ...
                return emailTemplate(emailTemplate.templates.EMAIL_OTP, { otp_code: findExistingOTP.otp_code })
                    .then(emailBody => {
                        nodemailer('fitnesswithgomzi@gmail.com', findExistingOTP.send_to, 'OTP for Login in Administration Panel', emailBody)
                            .catch(error => {
