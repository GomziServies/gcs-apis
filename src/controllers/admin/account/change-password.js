...
module.exports = async (req, res) => {
    ...
    let { password, old_password } = req.body, { adminAuthData } = req.headers
    ...
    try {
        ...
        // Match Hash
        let matchPassword = await bcryptjs.compare(old_password, userResult.password).then(() => true).catch(() => false)
        ...
        AdminRepo.findByIdAndUpdate({ _id: userResult._id }, {
            password,
            authToken: authToken
        }).exec().catch(error => req.logger.error(error))
